...
public class AlphavantageService implements StockQuotesService {
  ...
  private final String token = "N8TZGAUPL79C8P7B";
  ...
  public List<Candle> getStockQuote(String symbol, LocalDate from, LocalDate to)
    ...
    if (from.isAfter(to)) {
      throw new RuntimeException("Start date can't be after end dates");
    }

    List<Candle> candleReponse = new ArrayList<>();
    String url = buildUrl(symbol);
    // System.out.println(url);
    try {
      String apiResponses = restTemplate.getForObject(url, String.class);
      System.out.println(apiResponses);

      ObjectMapper objectMapper = new ObjectMapper();

      objectMapper.registerModule(new JavaTimeModule());

      Map<LocalDate, AlphavantageCandle> dailyResponses = objectMapper
          .readValue(apiResponses, AlphavantageDailyResponse.class).getCandles();

      for (LocalDate date = from; !date.isAfter(to); date = date.plusDays(1)) {
        AlphavantageCandle candle = dailyResponses.getOrDefault(date, null);
        if (candle != null) {
          candle.setDate(date);
          candleReponse.add(candle);
        }
      }
